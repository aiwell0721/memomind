...
class TestUserService:
    ...
    def test_create_user(self):
        """注册用户"""
        user_id = self.users.create_user("alice", "password123", "Alice Wang")
        assert user_id >= 1
    ...
    def test_create_user_unique_username(self):
        """用户名唯一性"""
        self.users.create_user("bob", "password123")
        with pytest.raises(Exception):
            self.users.create_user("bob", "password123")

    def test_get_user_by_username(self):
        """根据用户名获取用户"""
        self.users.create_user("charlie", "password123", "Charlie Li")
    ...
    def test_list_users(self):
        """列出所有用户"""
        self.users.create_user("user1", "password123", "User One")
        self.users.create_user("user2", "password123", "User Two")
        self.users.create_user("user3", "password123", "User Three")
    ...
    def test_update_user(self):
        """更新用户信息"""
        user_id = self.users.create_user("dave", "password123", "Dave Old")
    ...
    def test_update_user_noop(self):
        """更新用户（无变更）"""
        user_id = self.users.create_user("eve", "password123")
        result = self.users.update_user(user_id)
        assert result is False

    def test_delete_user(self):
        """删除用户"""
        user_id = self.users.create_user("frank", "password123")
    ...
    def test_delete_user_cleans_memberships(self):
        """删除用户清理成员关系"""
        user_id = self.users.create_user("grace", "password123")
...
class TestWorkspaceMembership:
    ...
    def test_add_member(self):
        ...
        user_id = self.users.create_user("alice", "password123")
    ...
    def test_add_member_invalid_role(self):
        """无效角色"""
        ws_id = self.ws.create_workspace("测试区")
        user_id = self.users.create_user("bob", "password123")
    ...
    def test_add_member_nonexistent_workspace(self):
        """添加到不存在的工作区"""
        user_id = self.users.create_user("charlie", "password123")
    ...
    def test_remove_member(self):
        ...
        user_id = self.users.create_user("dave", "password123")
    ...
    def test_list_members(self):
        ...
        u1 = self.users.create_user("alice", "password123")
        u2 = self.users.create_user("bob", "password123")
        u3 = self.users.create_user("charlie", "password123")
    ...
    def test_can_edit(self):
        """权限检查 - 编辑"""
        ws_id = self.ws.create_workspace("测试区")
        u_owner = self.users.create_user("owner1", "password123")
        u_editor = self.users.create_user("editor1", "password123")
        u_viewer = self.users.create_user("viewer1", "password123")
        u_none = self.users.create_user("none1", "password123")
    ...
    def test_can_view(self):
        """权限检查 - 查看"""
        ws_id = self.ws.create_workspace("测试区")
        u_member = self.users.create_user("member1", "password123")
        u_none = self.users.create_user("none1", "password123")
